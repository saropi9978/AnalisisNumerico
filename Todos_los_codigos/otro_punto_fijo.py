...
if __name__ == '__main__':
    ...
    f1 = lambda x: (math.cos(2*x)**2)    #Ejercicio 1
    ...
    f4D = lambda x: -((9*(x**2))/4) + 3*x  #Ejercicio 3 derivada
    f4 = lambda x: ((x**3)-(2*( x**2))+((4*x)/3)-(8/27)) #Ejercicio 3
    
    
    
    #(punto_fijo(f1, 0.51, 1e-8, f1D,200000))
    #(punto_fijo(f2, 1, 1e-16, f2D,5000))
    (punto_fijo(f4, 1, 1e-10, f3D,2000000))
    #print(nIteraciones)
    #print(errorI)
    # pylab.title("iteraciones vs error B.")
